...
for sy_file in "$TEST_CASES_DIR"/*.sy; do
    ...
    # 步骤 1: 生成LLVM IR
    echo -n "  生成IR... "
    if ./build/SysYAsmAutoTester "$sy_file" "$asm_file"; then
        echo "成功"
    else
        echo "失败"
        echo "  ⚠️ ASM生成异常"
    ...
    # # 步骤 2: 编译IR并链接库
    # echo -n "  编译链接... "
    # if clang -Wno-override-module "$ll_file" "$LIB_PATH" -o "$exe_file" &>/dev/null; then
    #     echo "成功"
    # else
    #     echo "失败"
    #     echo "  ⚠️ 编译/链接错误"
    ...
    # echo -n "  编译链接... "
    # # 编译sylib.c、LLVM IR，并链接
    # if clang -c sylib.c -o sylib.o \
    #     && clang -Wno-override-module -c "$ll_file" -o "${ll_file%.*}.o" \
    #     && clang "${ll_file%.*}.o" sylib.o -o "$exe_file"; then
    #     echo "成功"
    # else
    #     echo "失败"
    #     echo "  ⚠️ 编译/链接错误"
    ...
    if clang -g -c sylib.c -o sylib.o \
        && clang -g -Wno-override-module -c "$asm_file" -o "${asm_file%.*}.o" \
        && clang -g -Wl,-z,stack-size=104857600 "${asm_file%.*}.o" sylib.o -o "$exe_file"; then
        echo "成功"
    else
        echo "失败"
        echo "  ⚠️ 编译/链接错误"
