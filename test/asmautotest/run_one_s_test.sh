...
for sy_file in "$TEST_CASES_DIR"/*.sy; do
    ...
    # ll_file="$TEST_CASES_DIR/$base_name.ll"
    s_file="$TEST_CASES_DIR/$base_name.s"
    ...
    if clang-16 -g -target loongarch64-unknown-linux-gnu -mabi=lp64d --sysroot="$SYSROOT" -c sylib.c -o sylib.o \
        && clang-16 -g -target loongarch64-unknown-linux-gnu -mabi=lp64d --sysroot="$SYSROOT" -Wno-override-module -c "$s_file" -o "${s_file%.*}.o" \
