...
# 配置路径
DIFF_TEST_DIR="critical_cases"
TEMP_DIR="build/critical_diff_tests"
...
# echo "生成测试样例..."
# python3 generate_sysy_test.py || { echo "测试样例生成失败"; exit 1; }

# 移动和重命名生成的测试文件到diff_test_dir
# for test_file in sysy_*.c; do
#     if [ -f "$test_file" ]; then
#         # 提取文件名去掉.c后缀
#         base_name=$(basename "$test_file" .c)
#         # 重命名为.sy并移动
#         cp "$test_file" "$DIFF_TEST_DIR/${base_name}.sy"
#     fi
# done
