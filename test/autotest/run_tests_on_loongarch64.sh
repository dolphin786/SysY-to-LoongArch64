...
for sy_file in "$TEST_CASES_DIR"/*.sy; do
    ...
    步骤 1: 生成Loongarch64 ASM
    echo -n "  生成ASM... "
    if ./build/SysYAsmAutoTester "$sy_file" "$asm_file" 2&> /dev/null; then
        echo "成功"
    else
        echo "失败"
        echo "  ⚠️ ASM生成异常"
    ...
    if clang-16 -g -target loongarch64-unknown-linux-gnu -mabi=lp64d --sysroot="$SYSROOT" -c sylib.c -o sylib.o \
        && clang-16 -g -target loongarch64-unknown-linux-gnu -mabi=lp64d --sysroot="$SYSROOT" -Wno-override-module -c "$asm_file" -S -o "${asm_file%.*}.s" \
    ...
    /opt/cross-tools/loongarch64-unknown-linux-gnu/bin/objdump -d "$exe_file" > "$TEMP_DIR/$base_name.objdump"
